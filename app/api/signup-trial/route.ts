...
export async function POST(req: NextRequest) {
  try {
    ...
    const lookupKey = walletAddress || deviceId || email
    const existing = await db(`trial_users?or=(wallet.eq.${encodeURIComponent(lookupKey)},email.eq.${encodeURIComponent(email.toLowerCase().trim())})&select=*`)
    ...
    await db('trial_users','POST',{ device_id:deviceId||walletAddress||email, wallet:walletAddress||null, email:email.toLowerCase().trim(), trial_start:now, is_pro:false, created_at:now })

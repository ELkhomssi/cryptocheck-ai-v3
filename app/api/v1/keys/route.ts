...
function errorPayload(err: unknown): { message: string; log: Record<string, unknown> } {
  if (err && typeof err === 'object') {
    ...
    if (message) {
      return {
        message,
        log: { message, ...(code != null ? { code } : {}), ...(details != null ? { details } : {}), ...(hint != null ? { hint } : {}) },
      }
    }
  }
  if (err instanceof Error) {
    return { message: err.message, log: { message: err.message, stack: err.stack } }
  }
  return { message: 'Unexpected error', log: { err: String(err) } }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { message, log } = errorPayload(err)
    console.error('[api/v1/keys GET]', log)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { message, log } = errorPayload(err)
    console.error('[api/v1/keys POST]', log)
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const { message, log } = errorPayload(err)
    console.error('[api/v1/keys DELETE]', log)
    return NextResponse.json({ error: message }, { status: 500 })
  }
}

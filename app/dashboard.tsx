...
function ProMaxView({ isPro, onUpgrade }: { isPro: boolean; onUpgrade: () => void }) {
  ...
  const pmFeats = [
    { icon: '◈', title: 'Cluster Mapping', sub: 'SYBIL DETECTION', desc: 'GNN traces wallet relationships. Detect 100+ wallets from one entity.', stat: '847', statL: 'nodes mapped', accent: '#FFD700' },
    { icon: '◉', title: 'Heuristic Scoring', sub: 'DEEP LEARNING', desc: 'Trained on 523K+ contracts. Bytecode decompilation + temporal patterns.', stat: '523K+', statL: 'contracts', accent: '#20b2aa' },
    { icon: '◎', title: 'LP Forensics', sub: 'EXIT PREDICTION', desc: 'Predict liquidity removal 4-12 min before rug execution.', stat: '4-12', statL: 'min warning', accent: '#FFD700' },
  ];
  const pmComp = [
  ...
  const terminal = (h: number, blur: boolean) => (
    <div style={{ background:'#0a0a0a', border:'1px solid rgba(255,215,0,0.1)', borderRadius:8, overflow:'hidden', filter: blur?'blur(3px)':'none', opacity: blur?0.5:1, pointerEvents: blur?'none' as any:'auto' as any }}>
      ...
      <div ref={blur?undefined:logRef} style={{ height:h, overflowY:'auto', padding:'10px 14px', fontFamily:'IBM Plex Mono,monospace', fontSize:10, lineHeight:1.9, scrollbarWidth:'none' as any }}>
        {logs.map((l,i) => (
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginTop:20}}>
        {pmFeats.map((f,i) => (
          <div key={i} style={{background:'#0a0a0a',border:'1px solid rgba(32,178,170,0.1)',borderRadius:8,padding:'16px 14px'}}>
            <div style={{fontSize:8,fontWeight:700,letterSpacing:'0.12em',color:f.accent,marginBottom:6}}>{f.sub}</div>
            ...
            <div style={{borderTop:'1px solid rgba(255,255,255,0.04)',paddingTop:8,display:'flex',alignItems:'baseline',gap:4}}>
              <span style={{fontSize:18,fontWeight:800,color:f.accent}}>{f.stat}</span>
  ...
  return (
    <div style={{padding:20,display:'flex',flexDirection:'column',alignItems:'center'}}>
      <div style={{textAlign:'center',maxWidth:600,marginBottom:32,paddingTop:20}}>
        ...
        <h2 style={{fontSize:28,fontWeight:800,color:'#fff',margin:'0 0 8px',letterSpacing:'-0.02em',fontFamily:'IBM Plex Mono,monospace'}}>Deep Forensic <span style={{color:'#FFD700'}}>Intelligence</span></h2>
        <p style={{fontSize:12,color:'#6e7681',lineHeight:1.7,margin:'0 0 20px'}}>Graph Neural Networks + Contract Decompilation. Trained on 523K+ contracts. Predicts rug pulls before they execute.</p>
        <div style={{display:'inline-flex',alignItems:'center',gap:10,padding:'10px 20px',borderRadius:8,background:'rgba(255,215,0,0.04)',border:'1px solid rgba(255,215,0,0.1)',marginBottom:20}}>
          ...
          <div style={{textAlign:'left'}}><div style={{fontSize:10,color:'#FFD700',fontWeight:700}}>/month</div><div style={{fontSize:8,color:'#484f58',letterSpacing:'0.06em'}}>UNLIMITED AUDITS · 0% FEES</div></div>
        </div>
        <div><button onClick={onUpgrade} style={{padding:'12px 28px',fontSize:12,fontWeight:700,background:'linear-gradient(135deg,#FFD700,#d4af37)',border:'none',borderRadius:6,color:'#000',cursor:'pointer',fontFamily:'IBM Plex Mono,monospace',boxShadow:'0 0 20px rgba(255,215,0,0.15)',letterSpacing:'0.03em'}}>Upgrade to Pro Max</button></div>
      </div>
      <div style={{width:'100%',maxWidth:700,position:'relative',marginBottom:32}}>
        ...
        <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:8}}>
          <div style={{textAlign:'center'}}><div style={{fontSize:28,marginBottom:6}}>🔒</div><div style={{fontSize:10,color:'#FFD700',fontWeight:700,letterSpacing:'0.08em'}}>PRO MAX REQUIRED</div></div>
        </div>
      </div>

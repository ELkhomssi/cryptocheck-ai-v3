...
interface AuthModalProps {
  ...
  /** When set (e.g. `/signup`), opens in sign-up mode with terms gate. */
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:9999,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(16px)',display:'flex',alignItems:'center',justifyContent:'center',padding:16,fontFamily:'Inter,sans-serif'}}>
      <div onClick={e=>e.stopPropagation()} style={{width:'min(420px,95vw)',background:'#0a110b',border:'1px solid rgba(52,211,153,0.2)',borderRadius:14,overflow:'hidden',boxShadow:'0 0 60px rgba(52,211,153,0.1),0 32px 80px rgba(0,0,0,0.8)'}}>
          ...
          {sent ? (
            ...
            <>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
                <button onClick={()=>handleOAuth('google')} disabled={!!loading || (mode === 'signup' && !agreeToTerms)}
                  style={{width:'100%',padding:'11px',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:8,color:'#f0fdf4',fontSize:13,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10,transition:'all 0.15s',opacity:loading || (mode === 'signup' && !agreeToTerms) ? 0.7 : 1}}>
                ...
                <button onClick={()=>handleOAuth('github')} disabled={!!loading || (mode === 'signup' && !agreeToTerms)}
                  style={{width:'100%',padding:'11px',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:8,color:'#f0fdf4',fontSize:13,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10,transition:'all 0.15s',opacity:loading || (mode === 'signup' && !agreeToTerms) ? 0.7 : 1}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:8}}>
                ...
                {mode === 'signup' && (
                  <label style={{display:'flex',alignItems:'flex-start',gap:8,cursor:'pointer',fontSize:11,color:'#a7f3d0',lineHeight:1.45}}>
                    <input
                      type="checkbox"
                    ...
                    <span>
                      ...
                      <a href="/terms" target="_blank" rel="noopener noreferrer" style={{ color: '#34d399', fontWeight: 600 }} onClick={e => e.stopPropagation()}>
                        Terms of Service
                      </a>
                      .
                    </span>
                ...
                <button onClick={handleEmail} disabled={!!loading || (mode === 'signup' && !agreeToTerms)}
                  style={{width:'100%',padding:'11px',background:'linear-gradient(135deg,#34d399,#10b981)',border:'none',borderRadius:8,color:'#050a06',fontSize:13,fontWeight:700,cursor:'pointer',boxShadow:'0 0 16px rgba(52,211,153,0.3)',opacity:loading || (mode === 'signup' && !agreeToTerms) ? 0.7 : 1}}>
              ...
              <div style={{textAlign:'center',marginTop:14,fontSize:12,color:'#6ee7b7',opacity:0.7}}>
                ...
                <button onClick={()=>{setMode(mode==='signin'?'signup':'signin');setError('');setAgreeToTerms(false)}} style={{background:'none',border:'none',color:'#34d399',cursor:'pointer',fontSize:12,fontWeight:600}}>
        ...
        {/* Footer */}
        <div style={{padding:'12px 24px',borderTop:'1px solid rgba(52,211,153,0.08)',display:'flex',flexDirection:'column',alignItems:'center',gap:8}}>
          <div style={{display:'flex',justifyContent:'center',flexWrap:'wrap',gap:16}}>
            <a href="/privacy" style={{fontSize:11,color:'#6ee7b7',opacity:0.7,textDecoration:'none'}}>Privacy</a>
            <a href="/terms" style={{fontSize:11,color:'#6ee7b7',opacity:0.7,textDecoration:'none'}}>Terms of Service</a>
            <a href="/docs" style={{fontSize:11,color:'#6ee7b7',opacity:0.7,textDecoration:'none'}}>Docs</a>
          </div>
          <span style={{fontSize:10,color:'#6ee7b7',opacity:0.45}}>CryptoCheck AI, Inc. © 2026 · Delaware C-Corp</span>
        </div>
      </div>
    </div>
  )

...
type NeonForensicPanelProps = {
  ...
  /** Override default body padding (e.g. flush tables). */
...
const toneRing: Record<NonNullable<NeonForensicPanelProps['tone']>, string> = {
  neutral:
    'from-cyan-400/55 via-fuchsia-400/38 to-emerald-400/52 shadow-[0_0_20px_rgba(6,182,212,0.12)] hover:shadow-[0_0_32px_rgba(6,182,212,0.22)]',
  threat:
...
/**
 * Glassmorphism + animated neon triad border + Space Grotesk header (JetBrains for badge/subtitle).
 */
  ...
  return (
    ...
    >
      <div className="motion-safe:animate-neon-pulse motion-reduce:animate-none rounded-[15px] border border-white/[0.07] bg-black/50 backdrop-blur-xl transition-[transform,box-shadow] duration-300 group-hover/panel:-translate-y-0.5 group-hover/panel:shadow-[0_0_36px_rgba(6,182,212,0.18)]">
        <div
          className={`flex flex-wrap items-start justify-between gap-3 border-b border-cyan-500/10 px-5 py-4 ${headerClassName}`}
        >
          <div className="min-w-0 flex-1">
            <p className="font-space text-xs font-bold uppercase tracking-[0.22em] text-cyan-200/95 sm:text-sm">
              {title}
            </p>
            {subtitle ? (
              <p className="mt-1.5 font-mono-terminal text-xs font-medium leading-relaxed text-slate-400 sm:text-sm">
                {subtitle}
              </p>
            ) : null}
          </div>
          {badge ? (
            <span className="shrink-0 rounded-full border border-cyan-400/25 bg-gradient-to-r from-cyan-500/20 via-fuchsia-500/15 to-emerald-500/20 px-3 py-1.5 font-mono-terminal text-xs font-semibold uppercase tracking-wider text-cyan-100/90">
              {badge}
            </span>
          ) : null}
        </div>
        <div
          className={`px-4 pb-5 pt-4 text-sm leading-relaxed text-slate-200 sm:px-5 sm:pb-6 sm:pt-5 sm:text-base ${contentClassName}`}
        >
          {children}
        </div>
      </div>

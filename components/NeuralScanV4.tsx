          ...
          {activeTab === 'market' && (
            <div>
              ...
              <div style={{display:'flex',gap:8,marginTop:'12px'}}>
                <button onClick={()=>window.dispatchEvent(new CustomEvent('openChartSwap',{detail:{mint:result.mint,symbol:result.symbol,tab:'chart'}}))} style={{flex:1,padding:'8px 0',background:'rgba(56,189,248,0.08)',border:'1px solid rgba(56,189,248,0.2)',borderRadius:6,fontSize:'9px',color:'#38bdf8',cursor:'pointer',fontFamily:'IBM Plex Mono,monospace',fontWeight:700,letterSpacing:'0.06em'}}>📈 VIEW CHART</button>
                <button onClick={()=>window.dispatchEvent(new CustomEvent('openChartSwap',{detail:{mint:result.mint,symbol:result.symbol,tab:'swap'}}))} style={{flex:1,padding:'8px 0',background:'rgba(16,185,129,0.1)',border:'1px solid rgba(16,185,129,0.25)',borderRadius:6,fontSize:'9px',color:'#10b981',cursor:'pointer',fontFamily:'IBM Plex Mono,monospace',fontWeight:700,letterSpacing:'0.06em'}}>⚡ SWAP ON JUPITER</button>
              </div>
            </div>
          )}

 ...
 * Soft glass surface · ultra-thin border · institutional density.
 */
export function Panel({
  ...
  return (
    ...
    >
      <div className="flex h-9 shrink-0 items-center justify-between gap-2 border-b border-[var(--tit-border)] px-3">
      ...
      <div className={`tit-scroll min-h-0 flex-1 overflow-auto ${bodyClassName}`}>
        ...
        {state === 'error' ? (
          <p className="p-3 text-[0.72rem] text-[var(--tit-neg)]" role="alert">
            {errorMessage || 'Feed error — retry when ready.'}
          </p>
        ) : null}
        {state === 'empty' ? (
          <p className="p-3 text-[0.72rem] text-[var(--tit-text-1)]">{emptyMessage}</p>
        ) : null}

      ...
      <div className="flex shrink-0 flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-[var(--tit-border)] bg-[rgba(11,17,24,0.65)] px-3 py-2.5">
        <div className="min-w-0">
          <div className="flex items-baseline gap-2">
            <h2 className="tit-display truncate text-[1.05rem] font-semibold tracking-tight text-[var(--tit-text-0)]">
              {symbol ? `${symbol}/SOL` : 'Select symbol'}
            </h2>
            {symbol ? (
              <span className="tit-mono text-[0.58rem] uppercase tracking-[0.1em] text-[var(--tit-text-2)]">
                Primary
              </span>
            ) : null}
          </div>
          <p className="tit-mono mt-0.5 text-[0.55rem] text-[var(--tit-text-2)]">
            ...
            {bias !== 'neutral' ? ` · structure ${bias}` : ''}
          </p>
        </div>

        {lastPrice != null ? (
          <div className="ml-1">
            <p className="tit-mono text-[1.15rem] font-bold leading-none text-[var(--tit-text-0)]">
              ${lastPrice < 0.01 ? lastPrice.toPrecision(4) : lastPrice.toFixed(5)}
            </p>
            {changePct != null ? (
              <p
                className={`tit-mono mt-0.5 text-[0.7rem] font-semibold ${

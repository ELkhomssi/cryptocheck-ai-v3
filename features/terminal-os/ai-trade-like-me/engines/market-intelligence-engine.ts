/**
 * Market Intelligence Engine — aggregates live Terminal OS signals.
 * Never decides alone; feeds Decision Engine.
 */
...
export type MarketIntelInput = {
  ...
  /** Optional scan scores when gateway available */
  tokenScore?: number
  walletQuality?: number
  riskScore?: number
  securityBand?: MarketIntelSnapshot['securityBand']
}

/**
 * Build intel snapshot from live token + whale feed (no fabricated USD).
 */
export function buildMarketIntel(input: MarketIntelInput): MarketIntelSnapshot {
  const { token, whales = [] } = input
  const related = whales.filter(
    (w) =>
      w.assetSymbol.toUpperCase() === token.symbol.toUpperCase() ||
      w.chain === token.chain,
  )
  ...
  let whaleBias: MarketIntelSnapshot['whaleBias'] = 'neutral'
  ...
  const liqTrend: MarketIntelSnapshot['liquidityTrend'] =
  ...
  const tokenScore = input.tokenScore ?? clamp(50 + change * 1.2 + (token.buySellRatio - 1) * 15, 8, 95)
  const riskScore =
    input.riskScore ??
  ...
  const orderFlowBias: MarketIntelSnapshot['orderFlowBias'] =
    token.buySellRatio >= 1.15 ? 'buy' : token.buySellRatio <= 0.85 ? 'sell' : 'mixed'

  // Prediction upside from momentum + liquidity — labeled as model estimate in UI
  const predictionUpsidePct = Number(
    clamp(change * 0.35 + (whaleBias === 'accumulating' ? 8 : whaleBias === 'distributing' ? -6 : 0) + (liqTrend === 'increasing' ? 4 : 0), -40, 45).toFixed(1),
  )
...
export class MarketIntelligenceEngine {
  snapshot(input: MarketIntelInput): MarketIntelSnapshot {

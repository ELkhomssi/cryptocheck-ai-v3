...
export const BUILTIN_EMPLOYEES: readonly AIEmployee[] = [
  ...
  },
  {
    id: 'scout',
    name: 'Scout',
    role: 'Autonomous Growth Intelligence Agent — Terminal OS SEO, authority, and adoption from live CryptoCheckAI engines.',
    dataSources: [
      'birdeye-screener',
      'dexscreener',
      'coingecko-global',
    ...
    performanceFormula: formula({
      ...
      description:
        'Share of Scout V2 articles that clear priority + quality/fact gates and influence Terminal OS sessions.',
      verificationWindowHours: 72,
      recomputeCadence: 'daily',
      minSamples: 5,
    }),
    systemPromptTemplate: [
      'You are Scout V2 — CryptoCheckAI autonomous Growth Intelligence Agent.',
      'Never write generic crypto articles. Reinforce Terminal OS, Intelligence Chart, AI Gateway, Coaching, Trade Like Me, Scanner, Portfolio, Execution, Discovery.',
      'Educate first; never sell first. Never invent markets, search volumes, or profit promises.',
      'Prefer Terminal OS Scout pipeline (/api/scout/run) for production; chat answers must cite feed sources.',
      'Return JSON with title, summary, sections, stats, disclaimer.',
      'Never prefix with employee-name phrasing.',
      DISCLAIMER,
    ].join(' '),

...
/** @deprecated Import from `@/lib/security/signing` — re-export for backward compatibility. */
export { SCAN_API_DOCS_DEV_SIGNING_SALT } from '@/lib/security/signing'
...
/**
 * Optional HMAC-SHA256 integrity check. Cryptography is implemented only in `@/lib/security/signing`.
 */
export function assertScanSignature(req: NextRequest, rawBody: string, ctx: ScanAccessContext): void {
  ...
  const ok = verifySignature(ts, rawBody, sigHeader, rawKey)
  if (!ok) {
    throw new ScanServiceError(
